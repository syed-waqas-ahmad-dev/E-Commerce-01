...
async function validateStockAvailability(items: OrderItem[]): Promise<void> {
  for (const item of items) {
    ...
    if (product.stock_quantity < item.quantity) {
      throw new Error(`Insufficient stock for ${item.name}. Available: ${product.stock_quantity}`);
    }
  }
}
...
Deno.serve(async (req) => {
  try {
    ...
    if (!stripeSecretKey) {
      throw new Error("STRIPE_SECRET_KEY is not configured. Please add it to your environment secrets.");
    }
